            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                ...
                for index, access_token in enumerate(bots[:amount]):  # Limit to the specified amount
                    ...
                    future = executor.submit(handle_request, url, params, headers, filename, index,
                                             f"{G}Successfully followed user {user_id}.",
                                             f"{R}Failed to follow user", user_id=user_id, post_id=None)
...
def perform_reaction(filename, post_id, reaction_type, reaction_count, delay):
    def process_reaction(i, access_token):
        ...
        handle_request(url, params, headers, filename, i, f"{G}Successfully reacted.", "Failed to react.", user_id=None, post_id=post_id)
        time.sleep(delay)
    ...
    rp(f"{G}Starting reactions with {reaction_type} on post ID {post_id}...")
...
def add_comments(filename):
    ...
    def post_comment(index, access_token, post_id, comment_message):
...
async def leinathan_dev(file_path):
    bot = Bot(token=HELLO_TOKEN)
    try:
...
def boosting_menu():
    while True:
        ...
        rp(pan(f"{Y}[1] {G}AUTO REACT   [PAGE & NORMAL]\n"
                f"{Y}[2] {G}AUTO FOLLOW  [PAGE & NORMAL]\n"
                f"{Y}[3] {G}AUTO COMMENT [PAGE & NORMAL]\n"
                f"{Y}[4] {G}AUTO SHARE   [PAGE & NORMAL]\n"
                f"{Y}[0] {G}BACK\n",
